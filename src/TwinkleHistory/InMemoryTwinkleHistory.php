...
final class InMemoryTwinkleHistory implements TwinkleHistory
{
    ...
    {
        return count(array_filter($event->subjects, fn (CrossPackageReference $subject): bool => $subject->owner === 'sifrious/elwin' && $subject->type === 'twinkle'));
    }
    /** @param list<string> $types */
    private function hasSubject(EventEnvelope $event, string $owner, array $types): bool
    {
        return array_any($event->subjects, fn (CrossPackageReference $subject): bool => $subject->owner === $owner && in_array($subject->type, $types, true));
    }
}
