...
function twinkleEvent(string $id, string $type, int $version, array $subjects, string $time): EventEnvelope
{
    ...
    return new EventEnvelope($id, $type, 'sifrious/elwin', '1', $at, $at, $at, $subjects, null, 'twinkle-flow:1', [new CrossPackageReference('sifrious/elwin', 'conversation', 'chat:1')], null, ['twinkle_version' => $version, 'actor' => 'user:mary', 'rationale' => 'chosen by user']);
}
...
it('rejects conflicting event id reuse', function (): void {
    ...
    expect(fn () => $history->accept(twinkleEvent('event:5', 'twinkle.revised', 2, [twinkleRef('twinkle:1', '2')], '2026-08-30T12:00:00Z')))->toThrow(ConflictingTwinkleEvent::class);
});
