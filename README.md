...
`accept()` takes an `ObservationDraft` containing a source reference and name, canonical resource reference, producer reference and name, observation time, raw payload, and optional occurrence time, transformation lineage, metadata, and discoveries. Source, resource, and producer identities must be non-empty. Occurrence time cannot be later than observation time.
