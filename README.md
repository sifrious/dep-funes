...
`recordExtraction()` requires a `ProducerContext` containing producer identity and ingestion-run reference, then appends an idempotent succeeded, unsupported, or failed representation identified by observation, representation type, extractor, and extractor version. The store derives `inputHash` from the immutable observation payload. Failures carry a stable code, readable message, and structured details. `extraction()` and `extractions()` retrieve one version or the complete ordered set; a missing version returns `null`. Exact producer/run retries reuse the derived result and context; another run producing the same result appends context without duplicating the result. The configured database connection is read from `funes.connection`; `null` uses Laravel's default connection.
